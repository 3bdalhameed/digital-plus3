...
const fmtDateTime = (d?: string) =>
  d ? new Date(d).toLocaleString('ar-SA', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : '';
                  ...
                  <div className="pl-card__meta-row">
                    ...
                    {attachmentCount > 0 && (
                      <span className="pl-card__chip" title="مرفقات">
                        <span aria-hidden>📎</span> {attachmentCount.toLocaleString('ar-SA')}
                      </span>
                    )}
                  </div>
                </div>
      ...
      {totalPages > 1 && (
        <nav className="pl__pagination">
          ...
          <span className="pl__page-info">
            {data?.totalDocs?.toLocaleString('ar-SA') || 0} سجل · صفحة {currentPage.toLocaleString('ar-SA')} من {totalPages.toLocaleString('ar-SA')}
          </span>
        </nav>
      )}
    </div>
